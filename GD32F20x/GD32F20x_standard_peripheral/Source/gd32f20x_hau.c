...
{
    /* configure the algorithm, mode and the datatype */
...
/*!
    \brief      configure multiple or single DMA is used, and digest calculation at the end of a DMA transfer or not 
...
/*!
    ...
    \param[in]  interrupt: HAU interrupt flag status
                only one parameter can be selected which is shown as below
...
FlagStatus hau_interrupt_flag_get(uint32_t interrupt)
{
    ...
    if(RESET != ((HAU_INTEN & ret) & interrupt)){
        flag = SET;
    }
